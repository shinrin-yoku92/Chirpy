...
const insertRefreshTokens = `-- name: InsertRefreshTokens :one
...
type InsertRefreshTokensParams struct {
	Token     string
	UserID    uuid.UUID
	ExpiresAt time.Time
}

func (q *Queries) InsertRefreshTokens(ctx context.Context, arg InsertRefreshTokensParams) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, insertRefreshTokens, arg.Token, arg.UserID, arg.ExpiresAt)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.CreatedAt,
